 ...
 * @type number
 * @min 0
 * @desc This sets what icon to be used when an actor is banned.
 * @default 2
 *
 * @param ---Detail Window---
 * @default
...
Yanfly.Icon.PartyBanned = Number(Yanfly.Parameters['Banned Icon']);
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === 'BanActor') this.banActor(args, true);
    if (command === 'UnbanActor') this.banActor(args, false);
...
Game_Interpreter.prototype.banActor = function(args, value) {
    for (i = 0; i < args.length; i++) {
      var actorId = Number(args[i]);
      if ($gameActors.actor(actorId)) {
        $gameActors.actor(actorId)._banned = value;
      }
    }
};
...
Window_PartyMenuCommand.prototype.refuseCancel = function(actorId) {
    if ($gameParty._battleMembers.contains(actorId) && $gameActors.actor(actorId)._banned) return true
...
Window_PartySelect.prototype.drawActor = function(rect, actor) {
    ...
    this.drawBannedIcon(actor, rect);
};
...
Window_PartySelect.prototype.drawBannedIcon = function(actor, rect) {
    if (!actor._banned) return;
    var ix = rect.x + 2;
    var iy = rect.y + rect.height - Window_Base._iconHeight - 2;
    this.drawIcon(Yanfly.Icon.PartyBanned, ix, iy)
};
...
Window_PartyList.prototype.drawRestrictions = function(actor, rect) {
    ...
    if (actor._banned) {
      this.drawIcon(Yanfly.Icon.PartyBanned, wx, rect.y);
    }
};
